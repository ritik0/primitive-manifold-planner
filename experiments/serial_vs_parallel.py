...
def concluding_statement(parallel_rows: list[dict[str, object]], serial_rows: list[dict[str, object]]) -> str:
    ...
    time_delta_pct = 0.0 if serial_time <= 1e-9 else 100.0 * float((serial_time - parallel_time) / serial_time)
...
def run_batch(runs_per_mode: int, start_seed: int, output_path: Path) -> None:
    ...
    print("\nConclusion")
    print(concluding_statement(parallel_rows, serial_rows))
...
def main() -> None:
    ...
    parser.add_argument("--batch", action="store_true", help="Run the full serial/parallel comparison.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "examples" / "serial_vs_parallel_results.csv",
