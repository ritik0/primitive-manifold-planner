...
The required manifold sequence is fixed:

    left sphere -> fixed transfer plane -> right sphere

But evidence accumulation is not narrowly serial. Shared ambient proposals can
feed multiple stage-local explorers in parallel, while final route extraction
still enforces the required sequential order.
"""
...
def plan_fixed_manifold_multimodal_route(
    ...
    if robot is not None:
        from example_66_jointspace_planner import plan_fixed_manifold_multimodal_route_jointspace
...
def show_pyvista_unknown_sequence_route(
    ...
    plotter.add_text(
        f"Example 66: free-sequence multimodal planning\n{sequence_text}",
        font_size=12,
    )
    ...
    for stage_id in stage_ids:
        ...
        if len(stage_points) > 0:
            actor = add_points(
                ...
                color=colors[stage_id],
                size=5.0,
                label=f"{stage_id} evidence points",
            )
            if actor is not None:
                actor_groups["Evidence"].append(actor)
        ...
        if len(committed) > 0:
            actor = add_points(
                ...
                color="#0d47a1",
                size=10.0,
                label=f"{stage_id} committed nodes",
            )
            if actor is not None:
                actor_groups["Committed"].append(actor)
...
def show_unknown_sequence_route(
    ...
    for stage_id in stage_ids:
        draw_edge_segments(
            ax,
            result.stage_evidence_edges.get(stage_id, []),
            color=colors[stage_id],
            alpha=0.33,
            linewidth=1.6,
        )
...
def main():
    ...
    global SAFETY_MAX_TOTAL_ROUNDS, MIN_ROUNDS_BEFORE_SATURATION_CHECK, MIN_POST_SOLUTION_ROUNDS
    if args.max_rounds is not None:
        SAFETY_MAX_TOTAL_ROUNDS = max(4, int(args.max_rounds))
        MIN_ROUNDS_BEFORE_SATURATION_CHECK = min(MIN_ROUNDS_BEFORE_SATURATION_CHECK, max(4, SAFETY_MAX_TOTAL_ROUNDS // 2))
        MIN_POST_SOLUTION_ROUNDS = min(MIN_POST_SOLUTION_ROUNDS, max(2, SAFETY_MAX_TOTAL_ROUNDS // 4))
    if args.fast:
        SAFETY_MAX_TOTAL_ROUNDS = min(SAFETY_MAX_TOTAL_ROUNDS, 10)
        MIN_ROUNDS_BEFORE_SATURATION_CHECK = min(MIN_ROUNDS_BEFORE_SATURATION_CHECK, 6)
        MIN_POST_SOLUTION_ROUNDS = min(MIN_POST_SOLUTION_ROUNDS, 3)
