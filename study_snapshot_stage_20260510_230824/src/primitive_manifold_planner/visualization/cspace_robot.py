...
def show_cspace_robot_planning(
    *,
    ...
    show: bool = True,
    family_leaf_manifolds: Mapping[float, object] | None = None,
    selected_lambda: float | None = None,
    show_cspace_family: bool = False,
) -> Path | None:
    ...
    family_screenshot_path: Path | None = None
    if output_dir is not None:
        screenshot_path = Path(output_dir) / "cspace_environment.png"
        family_screenshot_path = Path(output_dir) / "cspace_family_environment.png"
        screenshot_path.parent.mkdir(parents=True, exist_ok=True)

    plotter = pv.Plotter(off_screen=not bool(show))
    plotter.set_background("white")

    family_leaf_manifolds = dict(family_leaf_manifolds or {})
    selected_lambda_value = None if selected_lambda is None else float(selected_lambda)
    neighbor_family_surface_points = 0
    neighbor_family_surface_count = 0
    if bool(show_cspace_family):
        for lam, manifold in sorted(family_leaf_manifolds.items(), key=lambda item: float(item[0])):
            lam_value = float(lam)
            if selected_lambda_value is not None and abs(lam_value - selected_lambda_value) <= 1.0e-6:
                continue
            points = _add_implicit_surface(
                plotter,
                pv,
                manifold,
                axes,
                color="#9bd7f0",
                opacity=0.10,
                label="sampled lambda family leaf",
            )
            if points > 0:
                neighbor_family_surface_count += 1
                neighbor_family_surface_points += int(points)

    surface_counts = {
        "left": _add_implicit_surface(plotter, pv, manifolds["left"], axes, color="#f97316", opacity=0.12, label="M_left"),
        "plane": _add_implicit_surface(plotter, pv, manifolds["plane"], axes, color="#5b21b6", opacity=0.35, label="selected M_lambda"),
        "right": _add_implicit_surface(plotter, pv, manifolds["right"], axes, color="#16a34a", opacity=0.12, label="M_right"),
    }
    family_surface_points_total = int(surface_counts["plane"] + neighbor_family_surface_points)
    family_leaves_displayed = int(1 + neighbor_family_surface_count) if bool(show_cspace_family) else 1

    full_line = _polyline_from_points(pv, theta_path)
    plotter.add_mesh(full_line, color="#111827", line_width=5, label="dense theta path")
    for stage, segment in _stage_segments(theta_path, labels):
        if len(segment) < 2:
            continue
        plotter.add_mesh(
            _polyline_from_points(pv, segment),
            color="#6d28d9" if stage in {"family", "plane"} else STAGE_COLORS.get(stage, "#dc2626"),
            line_width=9 if stage in {"family", "plane"} else 7,
    ...
    plotter.add_text(
        "C-space view: theta=[yaw, shoulder, elbow]\n"
        "Translucent sheets = sampled lambda family leaves\n"
        f"Highlighted sheet = selected fixed lambda = {selected_lambda_value:.6f}\n"
        "Path = dense theta trajectory"
        if selected_lambda_value is not None and bool(show_cspace_family)
        else "C-space view: theta=[yaw, shoulder, elbow]\n"
        "Surfaces are FK-pulled-back constraints; path = dense theta trajectory.",
        position="upper_left",
        font_size=10,
        color="black",
    )
    ...
    print("cspace_family_surface_points_total = " + str(family_surface_points_total))
    ...
    if show:
        ...
        plotter.close()
    if (
        bool(show_cspace_family)
        and screenshot_path is not None
        and family_screenshot_path is not None
        and screenshot_path.exists()
    ):
        family_screenshot_path.write_bytes(screenshot_path.read_bytes())
